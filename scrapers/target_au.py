...
class TargetScraper(BaseScraper):
    ...
    async def search_products(self, query: str) -> List[Product]:
        ...
        products = []
        search_url = f"https://www.target.com.au/search?text={query.replace(' ', '+')}"
    ...
    def _parse_product_item(self, item) -> Optional[Product]:
        ...
        try:
            ...
            # Stock status
            stock_elem = item.find('div', class_='stock-availability')
            in_stock = False
            if stock_elem:
        ...
        try:
            async with self.session.get(url, timeout=30) as response:
                if response.status == 200:
                    ...
                    # Stock status
                    stock_elem = soup.find('div', class_='stock-availability')
                    in_stock = False
                    if stock_elem:
                        stock_text = stock_elem.text.lower()
                        in_stock = 'in stock' in stock_text
            ...
            print(f"Error getting Target product details: {e}")
